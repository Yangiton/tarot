...
// 全息效果类型
export type HoloType = 'normal' | 'cosmos' | 'rainbow' | 'galaxy' | 'radiant' | 'pixel'
...
function updateEffects(el: HTMLElement, state: HoloFoilState, clientX: number, clientY: number) {
  ...
  if (state.glareEl) {
    ...
    state.glareEl.style.opacity = String(intensity * Math.min(1, fromCenter + 0.3))
  }
}
...
export const vHoloFoil: Directive<HTMLElement, HoloFoilOptions | HoloType | boolean | undefined> = {
  mounted(el, binding: DirectiveBinding<HoloFoilOptions | HoloType | boolean | undefined>) {
  ...
  updated(el, binding: DirectiveBinding<HoloFoilOptions | HoloType | boolean | undefined>) {
...
// 导出所有可用的全息类型
export const HOLO_TYPES: HoloType[] = ['normal', 'cosmos', 'rainbow', 'galaxy', 'radiant', 'pixel']
