...
<template>
  ...
  <nav class="fixed bottom-0 left-0 right-0 z-50 md:hidden">
    <div class="flex justify-around items-center h-[var(--nav-height)] px-4 bg-background/95 backdrop-blur-lg border-t border-gold/15">
      <button
        v-for="item in navItems"
        :key="item.path"
        :class="cn(
          'flex flex-col items-center gap-1 py-2 px-4 rounded-xl transition-all',
          isActive(item.path) ? 'text-gold bg-gold/10' : 'text-muted-foreground hover:text-gold/70'
        )"
  ...
  <nav class="fixed top-0 left-0 right-0 z-50 hidden md:block">
    <div class="flex justify-center items-center h-[var(--nav-height)] px-6 bg-background/95 backdrop-blur-lg border-b border-gold/15">
